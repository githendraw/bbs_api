...
const EXCEL_SHIPMENT=async(req,res)=>{
    ...
    const shipment = await models.Shipment.findAll(options);
    if (!shipment) return ReE(res,'Error',402);
    let Obj = shipment[0];
    console.log(Obj)
    let columns=[];
    ...
    let worksheet = workbook.addWorksheet("Shipment");
    worksheet.columns = [...columns];
    worksheet.addRows(shipment);
    res.setHeader(
        "Content-Type",
      ...
      res.setHeader(
        "Content-Disposition",
        "attachment; filename=" + "Shipment.xlsx"
      );
